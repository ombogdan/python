...
with open('./data.csv', encoding='utf8') as File:
    ...
    for i in range(len(rows)):
        ...
        kad_number = str(rows[i][14])
        if (rows[i][9])=="":
            percent= None
        else:
            percent = float(rows[i][9])*100
        if kad_number != " ":
            if(kad_number==str(rows[i-1][14])):

                ownership["right1_fio"] = str(rows[i][0])
                ownership["right1_inn"] = str(rows[i][2])
                ownership["right1_date_reestration"] = None
                ownership["right1_number"] = None
                ownership["right1_reestration_organization"] = None
                ownership["right1_passport"] = str(rows[i][13])
            ...
            purposeId = None
            purpose = None
            for i in range(len(rowsObjectInfo)):
                result = rowsObjectInfo[i].split(sep=";")
            ...
            d["contract_date_reestration"] = str(rows[i][6])
            d["contract_date_to"] = str(rows[i][7])
            ...
            rowsq = []
            with open('./find-parsel.txt', encoding='utf8') as Fil:
                center_data = {}
                read = Fil.readlines()
